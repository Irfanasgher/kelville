jQuery(function($) {
	...
	$(window).on('scroll', function(){
        if($(window).scrollTop() > 20)
        {
			...
			$('#header-waypoint-sticky').css('background-color', 'white')
			$('#header-waypoint-sticky').css('box-shadow', '0px 3px 8px -6px rgba(0, 0, 0, 0.43)')
			
        }else
        {
			$(".color-logo").css('display', 'none');
			$(".white-logo").css('display', 'block');
			$('#header-waypoint-sticky').css('background-color','#FA4164')
			$('#header-waypoint-sticky').css('box-shadow','none')
        }
